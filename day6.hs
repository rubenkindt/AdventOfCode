...
part1 :: Int
part1 =sum ([length . unique $ myAppend group|group <- input2Data input])

part2 :: Int
part2 =sum ([length $ unique2 alfabet group|group <- input2Data input])
--part2= 8

input2Data :: [String] -> [[String]]
input2Data [] = [[]]
input2Data (str:xs)= if str /= "" then myAdd (str) (input2Data xs) else []:(input2Data xs)
...
unique2 :: String -> [String] ->  String
unique2 allYes []  = allYes
unique2 alfab (str:xs) = unique2 newAlfab xs
 where
 newAlfab = if str =="" then alfab else myContains str alfab

myContains :: String -> String -> String
myContains "" cont = ""
myContains letter [] = ""
myContains letter (y:ys) = if member y letter then [y] ++ (myContains letter ys) else (myContains letter ys)

member :: Char -> String -> Bool
member _ [] = False
member char (s:xs)= if char == s then True else member char xs
