...
myCountCheckAll :: [String] -> Int
myCountCheckAll people = (foldr (\bool xs -> if bool then 1 + xs else xs) 0 (map (checkPerson neededValues) people))


neededValues :: [String]
neededValues = ["ecl","pid","eyr","hcl","byr","iyr","hgt"]

checkPerson :: [String] -> String ->  Bool
checkPerson [] str     = True 
checkPerson (x:xs) str =  (isInfixOf x str) && (checkPerson xs str)
{-
deepCheckPerson :: [String] -> String ->  Bool
deepCheckPerson [] str     = True 
deepCheckPerson (x:xs) str =  (if (isInfixOf x str) then findIndex x str  else False) && (deepCheckPerson xs str)

myParse :: String -> [String]
myParse str = map (mysplit) . words $ str

mysplit :: String -> char -> [String]
mysplit str char = groupBy (\a b -> b /= char) str
-}

checkMulti :: [String] -> [Bool]
checkMulti people = (map (checkPerson neededValues) people)
